                   ...
                   struct ticket_library *ticket_library_head)
{
	...
	printf("请输入您要购买的航班号\n");
	scanf("%s", vip_ticket_number);

	//printf("user_name->user_ticket_head.next:%s\n", user_name->user_ticket_head->next->number );
...
int return_ticket(struct userinfo *user_name,
                  struct ticket_library *ticket_library_head) //退票
{
	system("clear");
	char number[10];
	int insurance;
